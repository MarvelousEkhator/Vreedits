...
import {
  ...
  Image as ImageIcon, Shield, ShieldOff, Calendar, Tag, SlidersHorizontal, Flag,
  BookOpen, HelpCircle, ExternalLink,
} from "lucide-react";
import {
  Avatar, PostReactionPills, PostActionSheet, CropModal, OnboardingFlowModal,
  SettingsSidebar, AccessControlRow, relativeTime, genId,
  ROLE_COLORS, ACCENT_COLORS, CATEGORY_OPTIONS, SETTINGS_TITLES,
...
export default function CommunityDetailClient({ communityId, currentUserId }) {
  ...
  const [permChannelId, setPermChannelId] = useState(null);
  ...
  useEffect(() => {
    if (settingsPage === "roles" || settingsPage === "members" || settingsPage === "channels" || settingsPage === "channel-permissions") {
      loadRoles();
      loadMembers();
    }
  ...
  function selectPermChannel(c) {
    setPermChannelId(c.id);
    setPermStatus("");
    setPermDraft({
  ...
  async function handleSavePerms() {
    if (!permChannelId || !permDraft) return;
    setSavingPerms(true);
    setPermStatus("");
    const res = await fetch(`/api/communities/${communityId}/channels/${permChannelId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
    ...
    if (!res.ok) {
      setPermStatus(data.error || "Could not save permissions.");
      return;
    }
    setChannels((prev) => prev.map((c) => (c.id === permChannelId ? { ...c, ...(data.channel || permDraft) } : c)));
    setPermStatus("Saved.");
  }
      ...
      {view === "settings" && canManage && (
        ...
        >
          ...
          <div className="p-4" style={{ flex: 1 }}>
            ...
            {settingsPage === "channel-permissions" && (
              <div>
                <label className="text-xs" style={{ color: "var(--text-muted)" }}>Channel</label>
                <select
                  className="input pl-3 mt-1 mb-3"
                  value={permChannelId || ""}
                  onChange={(e) => {
                    const c = channels.find((x) => x.id === e.target.value);
                    if (c) selectPermChannel(c);
                    else { setPermChannelId(null); setPermDraft(null); }
                  }}
                >
                ...
                {permDraft && (
                  <div>
                    <AccessControlRow label="Who can view this channel?" value={permDraft.viewAccess} onChange={(v) => setPermDraft((p) => ({ ...p, viewAccess: v }))} roles={roles} />
                    <AccessControlRow label="Who can send messages?" value={permDraft.sendAccess} onChange={(v) => setPermDraft((p) => ({ ...p, sendAccess: v }))} roles={roles} />
                    <AccessControlRow label="Who can create threads?" value={permDraft.threadAccess} onChange={(v) => setPermDraft((p) => ({ ...p, threadAccess: v }))} roles={roles} />
                    <AccessControlRow label="Who can manage this channel?" value={permDraft.manageAccess} onChange={(v) => setPermDraft((p) => ({ ...p, manageAccess: v }))} roles={roles} />
                    {permStatus && <div className="text-xs mt-2" style={{ color: "var(--text-muted)" }}>{permStatus}</div>}
                    <button onClick={handleSavePerms} className="btn-primary mt-3" disabled={savingPerms}>
                      {savingPerms ? <Loader2 size={14} className="animate-spin" /> : "Save Permissions"}
                    </button>
                  </div>
                )}
              </div>
            ...
            {settingsPage && !["overview", "members", "invites", "channels", "roles", "danger", "threads", "rules", "onboarding", "community-guide", "channel-permissions"].includes(settingsPage) && (
